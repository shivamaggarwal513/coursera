...
def load_data():
    ...
    item_train = genfromtxt('./data/recsys_nn/content_item_train.csv', delimiter=',')
    user_train = genfromtxt('./data/recsys_nn/content_user_train.csv', delimiter=',')
    y_train    = genfromtxt('./data/recsys_nn/content_y_train.csv', delimiter=',')
    with open('./data/recsys_nn/content_item_train_header.txt', newline='') as f:    #csv reader handles quoted strings better
        item_features = list(csv.reader(f))[0]
    with open('./data/recsys_nn/content_user_train_header.txt', newline='') as f:
        user_features = list(csv.reader(f))[0]
    item_vecs = genfromtxt('./data/recsys_nn/content_item_vecs.csv', delimiter=',')

    movie_dict = defaultdict(dict)
    count = 0
#    with open('./data/recsys_nn/movies.csv', newline='') as csvfile:
    with open('./data/recsys_nn/content_movie_list.csv', newline='') as csvfile:
    ...
    with open('./data/recsys_nn/content_user_to_genre.pickle', 'rb') as f:
